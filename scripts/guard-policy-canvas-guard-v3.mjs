...
function checkMachinePolicyWiring() {
  ...
  const consistency = read('scripts/guard-policy-consistency-v2.mjs');
  const pkg = read('package.json');

  mustInclude(runner, 'scripts/guard-policy-canvas-guard-v3.mjs', 'guard bundle runner must execute canvas guard v3 before module guards');
  mustInclude(runner, 'scripts/guard-policy-consistency-v2.mjs', 'guard bundle runner must execute consistency guard v2 before module guards');
  mustInclude(consistency, 'checkNotificationSchemaPolicy', 'consistency v2 must check notification schema policy');
  mustInclude(consistency, 'checkRootCssImportContracts', 'consistency v2 must check root CSS import contracts');
  mustInclude(consistency, 'checkPackageLockPolicy', 'consistency v2 must check package-lock policy');
  mustInclude(pkg, 'guard-policy:guard', 'package.json must expose guard-policy:guard');
  mustInclude(pkg, 'guard-policy-canvas-guard-v3.mjs', 'guard-policy script must use canvas guard v3');
  mustInclude(pkg, 'guard-policy-consistency-v2.mjs', 'guard-policy script must use consistency guard v2');
}
