...
must(layout, "import './client-account-popover-ratio.css';", 'root layout must load account popover CSS');
...
must(css, '--client-account-popover-contract: canonical-console-account-dock', 'account CSS must use canonical account dock contract');
must(css, 'main.app-shell[data-client-console-shell="true"]', 'console shell scoped selector missing');
must(css, 'data-console-role="client"', 'role selector missing');
must(css, 'grid-template-columns: minmax(0, 3fr) var(--account-dock-width)', 'account dock must use shared grid');
...
mustNot(css, chipClass, 'account CSS still references retired chip class');
mustNot(css, chipMain, 'account CSS still references retired chip internals');
mustNot(css, staticClass, 'account CSS still references retired static chip');
mustNot(workspace, dynamicChip, 'workspace still mounts retired dynamic chip');
mustNot(workspace, staticChip, 'workspace still mounts retired static chip');
must(accountMenu, "type ConsoleRole = 'manager' | 'master' | 'client'", 'canonical account menu must support all roles');
must(accountMenu, "if (role === 'client') return 'Disputer workspace account'", 'Disputer account role label missing');
...
must(workspace, "import AccountMenu from './console/AccountMenu';", 'workspace must import canonical AccountMenu');
must(workspace, 'data-client-console-shell="true"', 'workspace shell marker missing');
must(workspace, 'className="main-area admin-monitor-main client-console-main"', 'main class missing');
must(workspace, 'data-console-header-grid="true"', 'grid marker missing');
must(workspace, '<AccountMenu role="client" mode="workspace"', 'workspace must mount canonical AccountMenu');
must(workspace, 'data-console-header-primary="true"', 'primary header marker missing');
