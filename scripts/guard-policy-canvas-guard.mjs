...
function checkCanvasCompleteness() {
  const path = 'docs/guard-policy-contradiction-resolution-canvas.md';
  const canvas = read(path);
  if (!canvas) {
    fail(`missing ${path}`);
    return;
  }
  ...
  for (const marker of requiredMarkers) {
    requireText(canvas, marker, `guard policy canvas missing marker: ${marker}`);
  }
}
...
function checkMachinePolicyWiring() {
  ...
  requireText(runner, 'scripts/guard-policy-canvas-guard.mjs', 'guard bundle runner must execute canvas guard before module guards');
  requireText(runner, 'scripts/guard-policy-consistency.mjs', 'guard bundle runner must execute consistency guard before module guards');
  requireText(consistency, 'checkNotificationSchemaPolicy', 'consistency guard must check notification schema policy');
  requireText(consistency, 'checkRootCssImportContracts', 'consistency guard must check root CSS import contracts');
  requireText(consistency, 'checkPackageLockPolicy', 'consistency guard must check package-lock policy');
  requireText(pkg, 'guard-policy:guard', 'package.json must expose guard-policy:guard');
}
...
function checkNotificationPolicy() {
  ...
  requireText(contract, 'strict-canonical-columns', 'notification contract must use strict canonical schema mode');
  requireText(readService, ".select('id,title,body,href,severity,read_at,created_at')", 'notification reads must use explicit canonical projection');
  requireText(readService, '.limit(', 'notification reads must cap result rows');
  requireText(writeService, 'recipient_role: input.recipientRole || null', 'notification writes must include canonical recipient_role');
  requireText(canonicalMigration, 'create table if not exists public.notifications', 'notification canonical table migration must exist');
  requireText(canonicalMigration, 'recipient_role text', 'notification canonical migration must define recipient_role');
  requireText(roleMigration, "notify pgrst, 'reload schema'", 'notification role migration must reload PostgREST schema cache');
...
function collectRequiredRootCssImports() {
  const guards = walk('scripts', (file) => file.endsWith('-guard.mjs'));
...
function checkSchemaChangingSqlPolicy() {
  const sqlFiles = walk('supabase/migrations', (file) => file.endsWith('.sql'));
  for (const file of sqlFiles) {
    const source = read(file).toLowerCase();
    const changesSchema = source.includes('create table') || source.includes('alter table') || source.includes('drop table');
    const isBaseCreate = file.endsWith('20260617133000_create_notifications.sql');
    if (changesSchema && !isBaseCreate && !source.includes("notify pgrst, 'reload schema'")) {
      fail(`${file} changes schema but does not reload PostgREST schema cache`);
    }
  }
}
