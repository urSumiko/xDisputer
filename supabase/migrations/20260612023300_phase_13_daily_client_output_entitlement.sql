-- Phase 13 — Daily client output entitlement wiring
-- Purpose:
--   - Let a client read their own daily output usage and next reset time.
--   - Let managers/master read daily usage from the same logic.
--   - Make generation output enforcement reset every US Eastern day.
--
-- Reset boundary:
--   America/New_York calendar date. This covers US Eastern time with DST.
...
begin
  ...
  return query select
    ...
    case
      ...
      else 'Daily output limit reached. Your allowance resets at the next US Eastern day.'
    end as message;
end;
$$;
