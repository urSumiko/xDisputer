-- Phase 11D/11E — Workspace Adoption
-- 11D: Migrate current profiles.manager_id into assignment records.
-- 11E: Add workspace-scoped read RPCs for manager/master account pages.
--
-- Safety rules:
--   - Additive only.
...
--   - Use new *_v1 RPC names to avoid return-type conflicts with partially-created functions.

create extension if not exists pgcrypto;

-- ============================================================
-- Phase 11D: idempotent migration from profiles.manager_id
-- ============================================================

create or replace function public.access_11d_migrate_profile_manager_assignments()
returns jsonb
language plpgsql
security definer
...
declare
  ...
  existing_assignment public.client_manager_assignments;
  inserted_assignment_id uuid;
begin
  default_workspace_id := public.access_default_workspace_id();

  -- Keep every existing account represented in the default workspace.
  ...
  -- Convert the current compatibility pointer into durable assignment records.
  ...
  loop
    perform public.access_ensure_default_workspace_membership(profile_record.manager_id);
    perform public.access_ensure_default_workspace_membership(profile_record.id);

    select ca.*
    into existing_assignment
    from public.client_manager_assignments ca
    where ca.workspace_id = default_workspace_id
      and ca.client_id = profile_record.id
      and ca.assignment_role = 'primary'
      and ca.assignment_status in ('pending', 'active')
    order by ca.created_at desc
    limit 1;

    if existing_assignment.id is not null and existing_assignment.manager_id <> profile_record.manager_id then
      update public.client_manager_assignments
      set assignment_status = 'transferred',
          revoked_by = auth.uid(),
      ...
      perform public.access_log_assignment_event(
        ...
        profile_record.id,
        existing_assignment.manager_id,
        'migrated_transfer_from_profiles_manager_id',
        existing_assignment.assignment_status,
        'transferred',
        jsonb_build_object('phase', '11D', 'new_manager_id', profile_record.manager_id)
      );
    ...
    if existing_assignment.id is null then
      ...
      values (
        ...
        jsonb_build_object('phase', '11D', 'source', 'profiles.manager_id')
      )
      ...
      perform public.access_log_assignment_event(
        ...
        profile_record.id,
        profile_record.manager_id,
        'migrated_from_profiles_manager_id',
        null,
        case when profile_record.account_status::text = 'pending_manager_approval' then 'pending' else 'active' end,
  ...
  return jsonb_build_object(
    ...
    'migrated_assignments', migrated_count,
    'assignment_events', event_count
  );
end;
$$;

grant execute on function public.access_11d_migrate_profile_manager_assignments() to authenticated;

-- ============================================================
-- Phase 11E: workspace-scoped summary RPC
-- ============================================================
...
-- ============================================================
-- Phase 11E: workspace-scoped directory RPC
-- ============================================================
...
-- Run the migration immediately after creating the helper.
