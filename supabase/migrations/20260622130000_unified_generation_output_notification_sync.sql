-- Unified generation -> output activity -> notification sync.
-- Root cause fixed: generated output, manager activity rows, and bell notifications were split
-- across multiple writers. This function makes the database the canonical sync owner.
  ...
  where recipient_user_id = manager_id_value
    ...
    and (
      ...
      or (
        ...
        and created_at >= run_row.created_at - interval '30 minutes'
      )
    )
  order by created_at asc
  limit 1;
...
begin
  ...
  loop
    ...
    where recipient_user_id = manager_id_input
      and created_by = activity_row.disputer_id
      and (
        href = exact_href
        or (
          href = filter_href
          and title = notification_title
          and created_at >= activity_row.created_at - interval '30 minutes'
        )
      )
    order by created_at asc
    limit 1;
...
-- Repair recent activity + notification rows using the canonical generated runs.
do $$
declare
  run_id uuid;
  manager_id_value uuid;
