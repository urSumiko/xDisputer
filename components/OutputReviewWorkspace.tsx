...
export default function OutputReviewWorkspace({ round, outputs, expectedRoutes, zipName, warnings, evidenceKey, evidence, onEvidenceChanged, onMessage, onZip, onReplace, mergedPdfName, mergedPdfReady, mergedPdfError, onMergedPdfDownload }: OutputReviewWorkspaceProps) {
  ...
  async function handleMergedPdf() {
    ...
    if (pdf) downloadBlob(effectivePdfName || pdf.name, pdf.blob);
  }

  useEffect(() => { setLocalFinalPdf(null); setLocalPdfError(''); }, [activeOutputs, evidenceKey, round]);
  useEffect(() => { if (!canPrepareLocalPdf || onMergedPdfDownload || localFinalPdf || localPreparing || mergedPdfReady) return; void prepareLocalMergedPdf(); }, [canPrepareLocalPdf, onMergedPdfDownload, localFinalPdf, localPreparing, mergedPdfReady]);
