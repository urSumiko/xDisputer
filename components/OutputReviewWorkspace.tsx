...
function blobToDataUrl(blob: Blob) { return new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onerror = () => reject(reader.error || new Error('Could not prepare PDF download.')); reader.onload = () => resolve(String(reader.result || '')); reader.readAsDataURL(blob); }); }
...
export default function OutputReviewWorkspace({ round, outputs, expectedRoutes, zipName, warnings, evidenceKey, evidence, onEvidenceChanged, onMessage, onZip, onReplace, mergedPdfName, mergedPdfReady, mergedPdfError, onMergedPdfDownload }: OutputReviewWorkspaceProps) {
  ...
  const effectivePdfName = mergedPdfName || localFinalPdf?.name || '';
  ...
  async function prepareLocalMergedPdf() {
    ...
    try {
      ...
      onMessage?.('Editable DOCX packet and merged PDF are ready.');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Merged PDF generation failed.';
      setLocalFinalPdf(null);
      setLocalPdfUrl('');
      setLocalPdfError(message);
      onMessage?.(`Merged PDF generation failed: ${message}`);
    } finally {
