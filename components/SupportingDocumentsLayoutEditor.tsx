...
type CanvasMeasureTargets = { grid: HTMLDivElement | null; frame: HTMLDivElement; leftRail: HTMLElement | null; rightRail: HTMLElement | null; maxWidth?: number };
...
const REVIEW_PACKET_MAX_CANVAS_WIDTH = 620;
...
function measuredCanvasSize({ grid, frame, leftRail, rightRail, maxWidth }: CanvasMeasureTargets): CanvasSize {
  ...
  const measuredWidth = Math.floor(Math.max(gridAvailable, frameAvailable));
  const cappedWidth = maxWidth ? Math.min(maxWidth, measuredWidth) : measuredWidth;
  const availableWidth = Math.max(MIN_CANVAS_WIDTH, cappedWidth);
...
export default function SupportingDocumentsLayoutEditor({ storageKey, assets, managerPanel, onChanged, onMessage }: Props) {
  ...
  const reviewPacketLayout = !managerPanel;
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      raf = window.requestAnimationFrame(() => {
        const next = measuredCanvasSize({ grid: grid.current, frame: frameElement, leftRail: leftRail.current, rightRail: controls.current, maxWidth: reviewPacketLayout ? REVIEW_PACKET_MAX_CANVAS_WIDTH : undefined });
  ...
  }, [managerPanel, reviewPacketLayout]);
  ...
  function doneCrop() { setTool('POSITION'); onMessage('Document crop saved for packet position 02.'); }
  function resetCrop() { edit({ cropX: 0, cropY: 0, cropWidth: 1, cropHeight: 1 }); setTool('POSITION'); onMessage('Selected document crop reset.'); }
  function rotate(direction: -90 | 90) {
    ...
    onMessage(`Document rotated ${clockwise ? 'right' : 'left'} 90°. Crop and size were preserved.`);
  }
  ...
  async function selectedCropAspect() {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error('Selected document could not be measured.'));
      image.src = url;
    });
  ...
  async function tightenSelectedFrame() {
    ...
    try {
      ...
      if (!aspect) {
        onMessage('Select a document before tightening the frame.');
        return;
      }
      ...
      onMessage('Frame tightened around the selected document.');
    } catch (error) {
      onMessage(error instanceof Error ? error.message : 'Selected document frame could not be tightened.');
    }
  }
  ...
  function finish(event: PointerEvent<HTMLElement>) {
    ...
    else if (drag.mode === 'MOVE') onMessage('Supporting document position saved for packet position 02.');
    else if (drag.mode === 'PAN') onMessage('Crop position saved.');
  }
  function resetPage() { const updated = resetSupportingPlacements(storageKey); latestAssets.current = updated; setWorkingAssets(updated); onChanged(updated); setTool('POSITION'); onMessage('Supporting Documents page returned to maximum-width alignment.'); }
