...
import { buildGenerationManifest, normalizeGeneratedOutputForManifest, type GenerationManifest } from '../lib/generation-manifest';
...
type ParsedWorkspaceSource = ReturnType<typeof parseSource>;
...
const CLIENT_INFO_HEADING = /^(?:CLIENT|CONSUMER|CUSTOMER|APPLICANT|PERSONAL)\s+(?:INFORMATION|INFO|DETAILS|DATA|PROFILE)$/i;

function clientLineKey(value: string) { return value.replace(/[\[\]{}()=*#_]+/g, ' ').replace(/[:\-|/]+$/g, '').replace(/[\-_]+/g, ' ').replace(/\s+/g, ' ').trim().toUpperCase(); }
function isClientInfoHeading(value: string) { return CLIENT_INFO_HEADING.test(clientLineKey(value)); }
function sanitizeParsedSource<T extends ParsedWorkspaceSource>(input: T): T {
  const clientNameKey = clientLineKey(input.name);
  const seen = new Set<string>();
  const address = input.address
    .map((line) => line.trim())
    .filter(Boolean)
    .filter((line) => !isClientInfoHeading(line))
    .filter((line) => clientLineKey(line) !== clientNameKey)
    .filter((line) => {
      const key = clientLineKey(line);
      if (!key || seen.has(key)) return false;
      seen.add(key);
      return true;
    });

  if (address.length === input.address.length && address.every((line, index) => line === input.address[index])) return input;
...
export default function LetterGeneratorWorkspaceV2({ accountEmail, accountRole = 'client' }: { accountEmail?: string | null; accountRole?: 'admin' | 'client' }) {
  ...
  const parsed = useMemo(() => sanitizeParsedSource(parseSource(source)), [source]);
  ...
  function recoverDraft() { if (!recoveryDraft) return; setSource(recoveryDraft.text); setRecoveryDraft(null); setNormalized(recoveryDraft.normalized); report(`${recoveryDraft.label} draft restored.`, 'success'); }

  async function makeZip(files: ReviewOutput[], notes: string[], date: string) { const [{ default: JSZip }, { addOrderedPacketFolders }] = await Promise.all([import('jszip'), import('../lib/ordered-packet-archive')]); const zip = new JSZip(); await addOrderedPacketFolders(zip, files, round, evidenceKey, parsed.name || 'Client', routes.map((route) => ({ type: route.type, bureau: route.bureau }))); return await zip.generateAsync({ type: 'blob' }); }
