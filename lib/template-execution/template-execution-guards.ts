...
export type TemplateExecutionGuardResult = { ok: boolean; blockers: string[]; warnings: string[]; summary: string };

function validationStatus(value: string | null | undefined) { return String(value || '').trim().toLowerCase(); }
function activeSlot(slotKey: string) { return isFtcEnabled() || !slotKey.endsWith('::EXHIBIT::FTC'); }

export function evaluateTemplateExecutionGuards(input: { round: Round; source: string; normalized: boolean; parsed: ParsedSource; routes: LetterRoute[]; references: LetterReference[]; templates: TemplateExhibits; resolver: ManagerTemplateResolver; }): TemplateExecutionGuardResult {
  ...
  for (const duplicate of input.resolver.duplicateActiveSlots()) warnings.push(`Multiple active assets exist for ${duplicate.slotKey}. Resolver will use the freshest active asset.`);

  for (const summary of input.resolver.templateSummary()) {
    if (!activeSlot(summary.slotKey)) continue;
  ...
  return { ok, blockers, warnings, summary: ok ? 'Template execution guards passed.' : `Template execution blocked: ${blockers.join(' ')}` };
}
