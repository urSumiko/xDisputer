...
const MIN_LETTER_TEXT_LENGTH = 240;
...
function isLetterKind(kind: TemplateDocumentKind) {
  return kind === 'DISPUTE_LETTER' || kind === 'LATE_PAYMENT_LETTER';
}
...
export async function assertTemplateRenderProof(input: TemplateRenderProofInput): Promise<TemplateRenderProof> {
  ...
  if (isLetterKind(input.kind) && renderedText.length < MIN_LETTER_TEXT_LENGTH) {
    blockers.push(`${input.kind} output appears to contain only a header or a collapsed body (${renderedText.length} text characters).`);
  }

  const routeNeedle = firstMeaningfulRouteNeedle(input.route);
  if (routeNeedle && !renderedText.includes(normalizeText(routeNeedle))) {
    const message = `${input.kind} output is missing the first routed account/inquiry marker "${routeNeedle}".`;
    if (isLetterKind(input.kind)) blockers.push(message);
