...
function exhibitAsset(round: Round, kind: ExhibitKind): ManagerTemplateFileAsset {
  return {
    id: `${round}-EXHIBIT-${kind}`,
    round_label: round,
    template_kind: 'EXHIBIT',
    letter_type: null,
    exhibit_kind: kind,
 ...
 * Required static exhibits resolve from the assigned manager cloud library first,
 * then use local browser storage only as a compatibility fallback.
 */
